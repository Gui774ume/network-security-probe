...
// List lists all SecurityProfiles in the indexer.
func (s *securityProfileLister) List(selector labels.Selector) (ret []*v1.SecurityProfile, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.SecurityProfile))
	})
	return ret, err
}
...
// List lists all SecurityProfiles in the indexer for a given namespace.
func (s securityProfileNamespaceLister) List(selector labels.Selector) (ret []*v1.SecurityProfile, err error) {
...
// Get retrieves the SecurityProfile from the indexer for a given namespace and name.
func (s securityProfileNamespaceLister) Get(name string) (*v1.SecurityProfile, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("securityprofile"), name)
	}
	return obj.(*v1.SecurityProfile), nil
}
